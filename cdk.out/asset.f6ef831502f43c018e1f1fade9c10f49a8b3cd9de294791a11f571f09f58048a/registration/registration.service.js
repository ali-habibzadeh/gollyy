...
class RegistrationService {
    ...
    async signUp({ email, username, password, phoneNumber }) {
        return new Promise((resolve, reject) => {
            const attrs = [
                new amazon_cognito_identity_js_1.CognitoUserAttribute({ Name: "email", Value: email }),
                new amazon_cognito_identity_js_1.CognitoUserAttribute({ Name: "phoneNumber", Value: phoneNumber }),
            ];
            this.userPool.signUp(username, password, attrs, [], (err, result) => err ? reject(JSON.stringify(err)) : resolve(result));
        });
    }
}
